...
This is the correlation matrix of the customer responses to the `r ncol(ProjectDataFactor)` attitude questions - which are the only questions that w  e will use for the segmentation (see the case):
...
1. Yes, we see high correlations between the following questions - 13&14(0.63), 16&17(0.62),28&16 (0.5), 28&20 (0.52), 26&22 (0.5), 20&16 (0.52), 9&3(0.5), 7&6(0.5), 26&9(0.5), 28&16(0.5), 20&27, 20&28

2. The correlations imply that the questions have the same underlying factor / meaning.
...
1. The table explains how well the components describe the whole data in order to clear up less correlated components and extract meaningful implications.

2. By the nature of the function, the line is decreasing. The shape is shaped this way because the first few components have more weights in explaning the data, which is why the line gets flatter. Therefore the shape cannot be increasing.

3. We want to see fewer yet significant componenets (a few components can explain most of the data). 
...
* Personal interest and passion in boating, it means that people who buy the boats are those who are really passionate about boating. Self motivation and interest play big parts to drive sales.

* We selected 5 factors, which include self fulfillment, lifestyle, brand percention, knowledge/expertise in boating, and price sensitivity.

* There factors are personality traits of people who buy our boats. The customers can then be segmented using these traits.

* There is a lot of business judgment and intuition that is needed to interpret the data. Just seeing the data alone would not be enough to help us in making decisions.
...
1. These types of customers are 
    Status Conscious
    Adventurous
    Brand Conscious 
    DIY
...
1. The numbers in the last table indicate how the different segments, on average, are different from the population mean on each of the different question. So if it is a large positive number, the particular question is significantly important for that segment.

2. We can see that based on the table and snake plot, segments 1,2, and 4 have similar responses and segments 3 and 5 are also clustered together

  Component 1: Status - Q11, performing repairs is strongly negative (-.41) 
  Component 2: Adventure - Q9, seeing boat as a status symbol is very negative (-.27) 
  Component 4: DIY - Q2, Price sensitivity over brand is very positive (.23)

  Component 3: Brand - Q9, seeing boat as a status symbol is very positive (.39) 
  Component 5: Price Sensitive - Q2, Price sensitivity over brand is very positive (.48)
...
1. The number of segments are subjective, but we selected five. This is on the basis of the "distance" plot, whereby the scree is about at 5 factors.

2. The segments that we found are Status, Adventure, Brand, DIY, and Price Conscious.
    
    
3. If we change the number of factors, the number of identifiable segments will probably change because now there are more combinations between the factors.
