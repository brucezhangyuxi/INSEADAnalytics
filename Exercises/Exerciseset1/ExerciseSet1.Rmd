...
**Your Answers here:**
<br>1, Because in dataSet1.R file mytickers contains a array with two data, the 1st is SPY and the 2nd is AAPL, so when myticker is called, all the data in the array will be downloaded.
<br>
<br>2, they are 595.5 , 0.148, 2.39 respectively 
<br>3. Sure, just add the ticker of Yahoo! in the myticker and call that number accordingly
...
**Your Answers here:**
<br>1,
<br>after transposedData = t(StockReturns), we can use nrow(transposedData) to calulate the rows and ncol(transposedData) to calculate the columns
<br>2, Yes
<br>transposedData = t(StockReturns)
<br>portfolio = apply(transposedData,2,mean)
...
**Your Answers here:**
<br>1. In data.R, the starting date is 2001-01-01. File does not give returns for the first day, so we have to set it to "2000-12-31"
<br>
<br>
<br>
...
**Your Answers here:**
<br>1. 
<br>It shows 20 , the Sum of the data in myData that is not the same as the first 20 data in StockReturns. In fact, none of the data is the same because of different data format, that's why it returns 1 in each row. 
<br>
<br>2. There is error, because the two array have different length, they can't perform numberial operation
